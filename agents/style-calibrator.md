...
**Objective numbers come from the script — never invent a score.** Every feature vector, clipped weight, stylometric distance, channel mix, combined score, and self-similarity band comes from:
`${CLAUDE_PLUGIN_ROOT}/scripts/sws_python.sh "$PAPER_ROOT" "${CLAUDE_PLUGIN_ROOT}/scripts/sws_stylometry.py" <mode> ...`
Modes: `--vector`, `--distance` (stylometric only), `--fit-weights` (clipped), `--fit-mix`, `--score`, `--rbf`, `--self-band` (see `references/stylometry-features.md`). The metric has TWO channels combined at the similarity level — `S = alpha*k_stylo + beta*haiku_sim` — never folded into one distance.
...
4. **Calibrate loop (per section type).**
   - Fit once: build positive pairs (same-author, incl. peer×peer) and negative pairs (author×field, peer×peer-of-other) and call `--fit-weights POS.json NEG.json --lam 0.3 --clip-lo 0.4 --clip-hi 2.5` for the clipped stylometric weights, THEN `--fit-mix POS.json NEG.json --weights W.json --pos-haiku ... --neg-haiku ...` for `{alpha, beta, gamma}`. Compute the self-band with `--self-band ... --alpha .. --beta .. --gamma .. --haiku-sims <csv>` (D10).
   - Each round: draft a candidate `profile.md`; generate the held-out section's prose ON ITS REAL OUTLINE/CLAIMS (content-controlled, D11) so only voice varies; run the AI-tells grep (R4 / cycle-#7) on the generated prose BEFORE scoring; vectorize generated + real held-out; get the combined voice score with `--score GEN.json HELDOUT.json --weights W.json --haiku-sim <median> --alpha .. --beta .. --gamma ..` (stylometric kernel + Haiku channel mixed at the similarity level).
