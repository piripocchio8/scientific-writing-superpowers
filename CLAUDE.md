...
5. **Python env:** Use Python ≥ 3.9 with `PyYAML` installed for the resolver unit tests; hooks and the filesystem-index utility are stdlib-only. Plugin scripts themselves resolve to `$PAPER_ROOT/.venv/bin/python` through `scripts/sws_python.sh` (per cycle-#6 D19/D20). The maintainer's local dev path is recorded in the gitignored `claude_memory/feedback_python_env.md`; never reference it in any committed plugin file.
...
## Inherited environment notes (carried from the manuscript session where SWS was conceived)

- The originating manuscript that seeded SWS lives in a local folder outside this repo (its path is kept out of version control). **Do not read it from here unless explicitly asked** — context bleed is a known issue, and SWS work was moved into this dedicated folder for compartmentalization.
